...
(async function () {
      ...
      this.addEventListener("load", function () {
        try {
          if (this._url.includes("/v1/user/info")) {
            ...
            data.result.name = "Phạm Ngọc Huy";
  ...
  async function handleKeySubmit() {
    ...
    submitBtn.innerHTML =
      '<span style="display:inline-block;width:16px;height:16px;border:2px solid rgba(255, 255, 255, 0.3);border-top-color:#fff;border-radius:50%;animation:spin 0.6s linear infinite;margin-right:8px;vertical-align:middle"></span> Đang xác thực...';
  ...
  async function checkSavedKeyInBackground() {
    ...
    try {
      ...
      if (txt.includes(savedKey)) {
        showCornerStatus(
          "✅",
          "Key hợp lệ! Chúc bạn xem phim vui vẻ! 🎬",
          true
        );
        return true;
      } else {
      ...
      showCornerStatus(
        "⚠️",
        "Không thể xác minh key. Đang hoạt động offline...",
        true
      );
      return true;
    }
